class Page < ActiveRecord::Base
  ...
  before_create :set_default_layout_for_page_type
  after_save :save_page_parts
  after_save :copy_status_to_children
  ...
  PAGE_SUB_TYPES = {
    :promo =>  %w{ template default },
    :email => %w{ html text }
  }
  
  
  def create_children_for_page_type
    if page_type
      PAGE_SUB_TYPES[page_type.to_sym].each do |sub_type|
        page = Page.new_with_defaults
        page.title = sub_type
        page.slug = "#{self.slug}-#{sub_type}"
        page.breadcrumb = "#{self.breadcrumb}-#{sub_type}"
        page.page_type = "#{page_type}_#{sub_type}"
        page.parent = self
  ...
  def set_default_layout_for_page_type
    if page_type and layout = Layout.find_by_name(page_type)
      self.layout = layout
    end
  end
    ...
    def copy_status_to_children
      if page_type == 'promo' or
          page_type == 'email'
        children.each do |child|
          child.status = status
          child.save
